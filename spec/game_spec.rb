...
describe 'A game of rock paper scissors' do
  ...
  describe 'playing' do
    ...
    context 'rock vs scissors' do
      ...
      it 'finishes the game after a winner is announced' do
        game = Game.new
        game.start
        game.play(:rock, :scissors)
        expect(game.finish).to be_truthy
      end
    end
    ...
    context 'a tie game' do
      it "returns 'Tie Game. Try again!' if a tie is detected" do
        game = Game.new
        game.start
        expect(game.play(:scissors, :scissors)).to eq('Tie game. Try again!')
      end

      it 'is not finished if a game is tied' do
        game = Game.new
        game.start
        game.play(:scissors, :scissors)
        expect(game.finish).to be_falsey
      end
    end
  end
